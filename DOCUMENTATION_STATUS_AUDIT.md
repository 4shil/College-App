...
| [TEST_RESULTS_SUMMARY.md](TEST_RESULTS_SUMMARY.md) | Doc | Needs decision | ✅ Yes | Repo has conflicting references: `books` vs `library_books` used in different places (e.g., [lib/backup.ts](lib/backup.ts)) | Decide canonical table name, then update doc + remaining scripts/code |
